    ...
    def __saveFile(self):
        with fitz.open(sourceFile) as doc:
            ...
            for page in doc:
                ...
                doc.save(f"{savepath}/output.pdf", garbage=4, deflate=True, clean=True)
        ...
        file.close()
        try:
            pdf.output(f"{savepath}edited.pdf", "F")
        except:
    ...
    try:
        with fitz.open(sourceFile) as doc1:
            with fitz.open(file) as doc2:
                ...
                savepath=filesaver()
                doc1.save(f"{savepath}/merged.pdf")
...
def filesaver():
    saveFile = askdirectory(parent=window, initialdir="/", title='Please select save directory')
    return saveFile
